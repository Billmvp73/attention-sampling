...
class FromTensors(MultiResolutionBatch):
    ...
    def patches(self, samples, offsets, sample_space, previous_patch_size,
        ...
        # old_offsets = offsets
        offsets = K.print_tensor(offsets, message="Value of original offsets")
        # offsets = tf.print(offsets, [offsets], "original offsets: ")
        ...
        # [old_offsets, sample_space, previous_patch_size, scale_samples, scale_offsets, space_available, step]
...
class MultiFromTensors(MultiResolutionBatch):
    def __init__(self, xs, ys, res_index):
        """Given input tensors for each level of resolution provide the
        patches.
    ...
    def patches(self, samples, offsets, sample_space, previous_patch_sizes,
                patch_size, fromlevel, tolevel):
        # # old_offsets = offsets
        # # offsets = K.print_tensor(offsets, message="Value of original offsets")
        # offsets = tf.Print(old_offsets, [offsets], "original offsets: ")
        # Make sure everything is a tensor
        sample_space = to_tensor(sample_space)
        previous_patch_sizes = [to_tensor(previous_patch_size) for previous_patch_size in previous_patch_sizes]
        patch_size = to_tensor(patch_size)
        shape_from = to_tensor(self._shape(fromlevel))
        shape_to = to_tensor(self._shape(tolevel))
